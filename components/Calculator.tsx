...
const Calculator: React.FC<CalculatorProps> = ({ onTrigger }) => {
  ...
  return (
    <div className="glass rounded-3xl p-6 shadow-2xl border border-slate-700/50">
      <div className="text-right mb-6 px-2">
        <div className="text-slate-500 text-sm h-6 mono overflow-hidden whitespace-nowrap">
          {equation}
        </div>
        <div className="text-5xl font-light mono text-white truncate">
          {display}
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {buttons.map((btn, i) => (
          <button
            ...
            onClick={btn.action}
            className={`
              h-16 rounded-2xl flex items-center justify-center text-xl font-medium transition-all
              ...
              ${btn.type === 'special' ? 'bg-slate-400/20 text-slate-300 hover:bg-slate-400/30' : ''}
            `}
          >
            {btn.label}
          </button>
